  ...
  const init = entities => {
    entities.forEach(entity => {
      ...
      if (entity.body.type === 'box') {
        if (!BOX_GEO) BOX_GEO = new PX.PxBoxGeometry({
          x: 0.5,
          y: 0.5,
          z: 0.5
        });
        ...
        const meshScale = new PX.PxMeshScale({
          x: 1,
          y: 1,
          z: 1
        }, {
        ...
        for (let i = 0; i < l2; i++) {
          iArr.push_back(g.faces[i].a);
          iArr.push_back(g.faces[i].b);
          iArr.push_back(g.faces[i].c);
        }
